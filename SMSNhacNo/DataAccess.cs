...
namespace SMSNhacNo
{
    class DataAccess
    {
        ...
        public static DataTable GetSmsDataMCW(string statementMonth, string cardBrn)
        {
            DataTable data = new DataTable();
            OracleConnection conn = null;
            try
            {
                conn = OracleDBConnection.OpenConnectionDB("CW_IM_STAN");
                OracleCommand cmd = new OracleCommand("GET_SMS_NHAC_NO_SAO_KE_68", conn);
            ...
            {
                _logSMSNhacNoSaoKe.WriteLog("Error GET_SMS_NHAC_NO_SAO_KE_68 at DataAccess, " + ex.Message);
                if (conn != null)
                    conn.Close();
                data.Clear();
                return data;
        ...
        public static int update_Data_Notify_Send_SMS_Statement(String smsMonth,String cardbrn, String cyccDay)
        {
            OracleConnection conn = new OracleConnection();
            try
            {
                conn = OracleDBConnection.OpenConnectionDB("CW_DW");
                string str = "UPDATE SMS_NHAC_NO " +
                "SET SMS_DETAIL = (CASE WHEN ID LIKE '%START%' THEN 'Bat dau' ELSE 'Ket thuc' END) || ' goi SMS sao ke ' || " + cardbrn + cyccDay + " " + smsMonth + ", "  + 
                "GET_TRANS_DATETIME = TO_CHAR(SYSDATE, 'YYYYMMDD') || (CASE WHEN ID LIKE '%START%' THEN '000000' ELSE '999999' END), " +
                "INSERT_TRANS_DATETIME = 0, " +
                "CARD_BRN = '" + cardbrn + "', " +
                "SMS_MONTH = " + smsMonth + ", " +
                "ACTION_TYPE = 'N' " +
                "WHERE SMS_TYPE = 'DEBT01' AND Regexp_Like(ID, 'START|END')";
                OracleCommand cmd = new OracleCommand(str, conn);
                cmd.CommandType = CommandType.Text;
                int rowsUpdated = cmd.ExecuteNonQuery();
                if (rowsUpdated == 0)
                    _logSMSNhacNoSaoKe.WriteLog("Update record table SMS_NHAC_NO failed because " + rowsUpdated);
                else
                    _logSMSNhacNoSaoKe.WriteLog("Update record table SMS_NHAC_NO");
            ...
            {
                _logSMSNhacNoSaoKe.WriteLog("Update record DB SMS_NHAC_NO failed because " + ex.Message);
                if (conn != null)
                    conn.Close();
                return -1;
            }
        ...
        public static List<ObjInsertSMSSaoKe> getListSMSNhacNoSaoKe(string p_cardBrn, string p_stateMonth, string p_smsType)
        {
            ...
            try
            {
                ...
                string str = @"select ID, SMS_TYPE, SMS_DETAIL, DEST_MOBILE, GET_TRANS_DATETIME
                                      ...
                                      , TOL_BAL_IPP, VIP, CIF_VIP, CARD_NO, LOC, ID_STATEMENT, STATEMENT_AMT
                               from SMS_NHAC_NO 
                               where ACTION_TYPE = 'N' 
                                     and SMS_MONTH = " + p_stateMonth + @"
                                     and CARD_BRN = '" + p_cardBrn + @"'
                                     and SMS_TYPE = '" + p_smsType + "'" +
                                     " ORDER BY GET_TRANS_DATETIME ASC";
                ...
                if (objReader.HasRows)
                {
                    while (objReader.Read())
                    {
                        ...
                        string statementAmt = "";
                        if (objReader["STATEMENT_AMT"] != null)
                            closingBal = objReader["STATEMENT_AMT"].ToString().Trim();
                        ...
                        ObjInsertSMSSaoKe obj = new ObjInsertSMSSaoKe(id, smsType, smsDetail, destMobile,
                            ...
                            minPayment, actionType, tolBallIpp, vip, cifVip, cardNo, loc, idStatement, statementAmt);
        ...
        //TANVH1 SendSMSDueDate
        public static List<ObjInsertSMSDueDate> getListSMSNhacNoDueDate(string p_dueDate, string p_stateMonth, string p_smsType)
        {
            ...
            try
            {
                ...
                if (objReader.HasRows)
                {
                    while (objReader.Read())
                    {
                        ...
                        ObjInsertSMSDueDate obj = new ObjInsertSMSDueDate(id, smsType, smsDetail, destMobile,
                            dateTime, insertTransDate, pan, cardBrn, cardType, smsMonth, closingBal, dueDate,
                            minPayment, actionType, tolBallIpp, vip, cifVip, cardNo, loc, idStatement, statementAmt);
            ...
            {
                _logSMSNhacNoDueDate.WriteLog("Get data DB SMS_NHAC_NO failed because " + ex.Message);
                if (conn != null)
                    conn.Close();
                return null;
            }
        ...
        public static DataTable getListSMSExportNhacNoDueDate(string p_dueDate, string p_stateMonth, string p_smsType)
        {
            DataTable data = new DataTable();
            OracleConnection conn = new OracleConnection();
            try
            ...
            {
                _logSMSNhacNoDueDate.WriteLog("Get data export DB SMS_NHAC_NO failed because " + ex.Message);
                if (conn != null)
                    conn.Close();
                return null;
            }
        ...
        public static DataTable getListSMSExportThongBaoSaoKe(string p_cardBrn, string p_stateMonth, string p_smsType)
        {
            DataTable data = new DataTable();
            OracleConnection conn = new OracleConnection();
            try
            ...
            {
                _logSMSNhacNoSaoKe.WriteLog("Get data export DB SMS_NHAC_NO failed because " + ex.Message);
                if (conn != null)
                    conn.Close();
                return null;
            }
        ...
        public static void UpdateStatusSMSNhacNoDueDate(string id, string dueDate, string stateMonth, string smsType, string status)
        {
            OracleConnection conn = new OracleConnection(); // C#
            try
            {
                ...
                int rowsUpdated = cmd.ExecuteNonQuery();
                if (rowsUpdated == 0)
                    _logSMSNhacNoDueDate.WriteLog("UpdateStatusSMSNhacNoDueDate: Update record DB SMS_NHAC_NO failed because " + rowsUpdated);
                else
                    _logSMSNhacNoDueDate.WriteLog("UpdateStatusSMSNhacNoDueDate: Update stattus record DB SMS_NHAC_NO");

                conn.Close();
            }
            catch (Exception ex)
            {
                _logSMSNhacNoDueDate.WriteLog("UpdateStatusSMSNhacNoDueDate: Update record DB SMS_NHAC_NO failed because " + ex.Message);
                if (conn != null)
                    conn.Close();
            }
        }
        ...
        public static DataTable GetSmsDataDueDateMCW(string settlMonth, string dueDate)
        {
            DataTable data = new DataTable();
            OracleConnection conn = null;
            try
            {
                conn = OracleDBConnection.OpenConnectionDB("CW_IM_STAN");
                OracleCommand cmd = new OracleCommand("GET_SMS_NHAC_NO_DUE_DATE_68", conn);
            ...
            {
                _logSMSNhacNoSaoKe.WriteLog("Error GET_SMS_NHAC_NO_DUE_DATE_68 at DataAccess, " + ex.Message);
                if (conn != null)
                    conn.Close();
                data.Clear();
                return data;
        ...
        public static int update_Data_Notify_Send_SMS_DueDate(String smsMonth, String duedate)
        {
            OracleConnection conn = new OracleConnection();
            try
            {
                conn = OracleDBConnection.OpenConnectionDB("CW_DW");
                string str = "UPDATE SMS_NHAC_NO " +
                "SET SMS_DETAIL = (CASE WHEN ID LIKE '%START%' THEN 'Bat dau' ELSE 'Ket thuc' END) || ' goi SMS due date ' || " + duedate + ", " +
                "GET_TRANS_DATETIME = TO_CHAR(SYSDATE, 'YYYYMMDD') || (CASE WHEN ID LIKE '%START%' THEN '000000' ELSE '999999' END), " +
                "INSERT_TRANS_DATETIME = 0, " +
                "DUE_DATE = " + duedate + ", " +
                "SMS_MONTH = " + smsMonth +", " +
                "ACTION_TYPE = 'N' " +
                "WHERE SMS_TYPE = 'DEBT02' AND Regexp_Like(ID, 'START|END')";
                OracleCommand cmd = new OracleCommand(str, conn);
                cmd.CommandType = CommandType.Text;
                int rowsUpdated = cmd.ExecuteNonQuery();
                if (rowsUpdated == 0)
